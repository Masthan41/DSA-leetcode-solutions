...
Approach: Dynamic Programming (Using Digit DP to count specific digit patterns (peaks and valleys) across a numeric range.)

Time complexity: O(d*10*10) where d will always be <= 16
                so, O(16*10*10) => O(1600) which is efficient for our constraints.
                => O(1) time complexity for the DP part.

Space complexity: O(d*10*10) where d will always be <= 16
                so, O(16*10*10) => O(1600) which is efficient for our constraints.
                => O(1) Space complexity.
*/

class LC3753_TotalWavinessII {
    String s;
    int n;
    ...
    public static void main(String[] args) {
        LC3753_TotalWavinessII obj = new LC3753_TotalWavinessII();
        long num1 = 100, num2 = 2000;
